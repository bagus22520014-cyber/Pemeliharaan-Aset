...
import { FaPlus, FaEdit, FaKey, FaEye, FaEyeSlash } from "react-icons/fa";
...
export default function UserManagement() {
  ...
  const handleCreateUser = async () => {
    ...
    setCreateError(null);
    try {
      const bebanString = formRole === "user" ? selectedBases.join(",") : null;
      await createUser({
        nama: formNama,
  ...
  const handleEditBeban = async () => {
    ...
    setEditError(null);
    try {
      const bebanString = editSelectedBases.join(",");
      await updateUser(editingUser.id, { beban: bebanString });
  ...
  const handleResetPassword = async (userId) => {
    setResetStatus((prev) => ({ ...prev, [userId]: "loading" }));
    try {
  ...
  const openEditBebanPopup = (user) => {
    setEditingUser(user);
    const currentBases = user.beban
      ? user.beban.split(/[,;|]/).map((s) => s.trim())
      : [];
                    ...
                    <div className="max-h-48 overflow-y-auto border border-gray-300 rounded-lg p-3 space-y-2">
                      {bebans.map((b) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={selectedBases.includes(b.kode)}
                            onChange={() => toggleBase(b.kode)}
                ...
                <div className="max-h-64 overflow-y-auto border border-gray-300 rounded-lg p-3 space-y-2">
                  {bebans.map((b) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={editSelectedBases.includes(b.kode)}
                        onChange={() => toggleBase(b.kode, true)}
                        className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                      />
                      <span className="text-sm text-gray-700">{b.kode}</span>
                    </label>
                  ))}
                </div>
      ...
      <Confirm
        ...
        onConfirm={() => {
          if (confirmDialog.type === "create") {
            ...
            handleResetPassword(confirmDialog.user.id);
          }
